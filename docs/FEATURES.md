...
- **Log Time**: From Reports → Task Breakdown, manually record meetings or short work without running a timer (counts toward focus totals and streaks, not completed focus cycles).
...
- **Setup without a picker**: Copy configuration (primary), Install in Cursor, and Setup Guide for Claude Code, Codex, Gemini CLI, and other MCP clients.
